...
V prejšnjem poglavju predlagane deterministične nastavke dinamike sedaj zaporedno vstavljamo v splošno reakcijsko-difuzijsko enačbo (\ref{dinamicna-splosna}) in jo numerično rešujemo pri robnih pogojih (\ref{rd-robni}). Omejimo se na eno dimenzijo, $x$, ki nam pove kje v prerezu vrtače se nahajamo, višino $h(\mathbf{x},t)$, ki nam pove višino površja v točki $\mathbf{x}$ ob času $t$ in čas $t$. Rešitve prikažemo in komentiramo njeno časovno dinamiko. Izbrani robni pogoji
