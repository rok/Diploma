...
Vrtače so zaobljene lijakaste globeli, globine nekaj metrov in premera nekaj deset metrov. Prvi jih leta 1893 opiše in poimenuje Cvijić \cite{cvijic1893}. Najdemo jih na starih kraških poljih in planotah po celotnem Dinarskem krasu. Medtem ko v literaturi ocene časa potrebnega za nastanek vrtač ne najdemo, v nadaljevanju ta čas ocenimo na 0,5 do 3,5 milijona let. Primer starega polja z razvitimi vrtačami: (Slika \ref{fig:vrtace-bpetrovac}). 
  ...
  \begin{figure}[h]
    \begin{center}
      \includegraphics[width=7cm]{slike/bpetrovac}
    \end{center}
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=6.5cm]{slike/vrtaca-ford-williams.jpg}
  \end{center}
...
Porazdelitev efektivnih polmerov pokažemo na histogramu (Slika \ref{fig:menisija-polmeri-hist}).
  \begin{figure}[h!]
    \begin{center}
      \includegraphics[width=15cm]{slike/menisija-polmeri-hist}
    \end{center}
    \caption{Polmeri konkavnih objektov v Menišiji. Povprečen efektivni polmer objekta je $\bar r_{eff}=26m$, standardna deviacija efektivnega polmera pa je $\sigma_{r_{eff}}=24m$. [TODO]Videti je kot Maxwellova porazdelitev: $ N = A r_{eff}^2 e^{r_{eff}^2/2 \sigma_{reff}^2}$
Je to res? Se da nafitati? $\sigma_{eff}$[TODO]}
    \label{fig:menisija-polmeri-hist}
  \end{figure}

Povprečen efektivni polmer objekta je $\bar r_{eff}=26m$, standardna deviacija efektivnega polmera pa $\sigma_{r_{eff}}=24m$. Za $71\%$ objektov velja: $2m < r_{eff} < 50m$.
Zavržemo objekte izven tega intervala in z ročnim pregledom posameznih vrtač preverimo, da je večina najdenih objektov res vrtač.
...
Po dolgem času rast površja preide v zasičeni režim, v katerem je površje popolnoma korelirano in se eksponentna rast hrapavosti ustavi, ter doseže zasičeno vrednost $w_{sat}(L)$, ki je od velikosti sistema $L$ odvisna takole:
  ...
  \begin{equation}
    ...
    \label{alfa}
  \end{equation}
Eksponent hrapavosti $\alpha$ je tudi odvisen od korelacijskega mehanizma in nam pove kako je hrapavosti zasičenega površja odvisna od njegove velikosti.
[TODO]Neke vrste fraktalna odvisnost, $\alpha$ fraktalna dimenzija.[TODO]
...
\begin{equation}
  ...
  \label{KPZ3}
\end{equation}
katere formalna rešitev se da dobiti kot:
\begin{equation}
...
[TODO]
Še enkrat primerjajte statističen opis populacije vrtač in deterministični opis vrtače. V obeh primerih:
\begin{equation}
  \frac{ \partial h(t,x) }{ \partial t} = D \Delta h + F(h).
  \label{dinamicna-splosna3}
\end{equation}

1.) $F(h) = \eta h$ ($\eta$ = gaussov šum)
2.) $F(h)$ nelinearna a deterministična
[TODO]
