    ...
    % prazna stran
        ...
        \[
          f(u) \propto \left \{ \begin{array}{lr} u^{\beta} & \ u\ll 1 \\
            ...
            Če sedaj izrišemo več različnih razvojev površja na en graf ((Slika \ref{fig:barabasi}) iz \cite{barabasi1995fractal}), dobimo jasnejšo predstavo o funkciji skaliranja. Če se točki $(t_x,w(t_x))$ približamo z leve, vidimo, da po (\ref{beta}) velja $w(t_x) \sim t_x^\beta$. Hkrati pa, da če se isti točki približujemo z desne po (\ref{alfa}) velja $w(t_x) \sim L^\alpha$. Torej $t_x^\beta \sim L^\alpha$ in po (\ref{z}) sledi zakon o skaliranju:
            ...
            Ko se del kamnine raztopi, pomaknemo površino površja v smeri normale (Slika \ref{fig:kpz}), torej se površje v smeri osi h v času $\delta t$ pomakne za $\delta h = \sqrt{ (v \delta t)^2 + (v \delta t \nabla h)^2}$. Če je $|\nabla h| \ll 1$, dobimo: 
         ...
         \[ \frac{\partial h(t,x)}{\partial n} = 0, x \in \partial D\]
          ...
          Če izberemo $F(h) = a \cdot h$, dobimo:
            ...
            Tak sistem nam da eksponentno rast, kot prikazano na (Slika \ref{difuzija-eksponentna-rast}).

            \begin{figure}[H]
              \begin{center}
                \includegraphics[width=10cm]{slike/difuzija-eksponentna-rast}
              \end{center}
              \caption{Vzeli smo $D=10$, $a=1$ in majno začetno porazdelitev, ki povzroči začetek rasti - slika a) \cite{kandler2010population}}
              \label{fig:difuzija-eksponentna-rast}
            \end{figure}

            Eksponentna rast za vrtače ni primeren model, saj je v naravi ne opazimo.


          \subsection{Logistična rast}

          Če izberemo $F(h) = h \cdot (1 - \frac{h}{K})$, dobimo:
            ...
            \begin{figure}[H]
              \begin{center}
                \includegraphics[width=10cm]{slike/difuzija-logisticna-rast}
              \end{center}
              \caption{Vzeli smo $D=10$, $a=1$, $K=100$ in majno začetno porazdelitev, ki povzroči začetek rasti - slika a) \cite{kandler2010population}}
          ...
          Če izberemo $F(h) = (K - h)$, dobimo:
            ...
            \begin{figure}[H]
              \begin{center}
                \includegraphics[width=10cm]{slike/difuzija-omejena-eksponentna-rast}
              \end{center}
              \caption{Vzeli smo $D=10$, $a=1$, $K=100$ in majno začetno porazdelitev, ki povzroči začetek rasti - slika a) \cite{kandler2010population}}
          ...
          Če izberemo $F(h) = h \cdot a_0 e^{-a t}$, dobimo:
            ...
            \begin{figure}[H]
              \begin{center}
                \includegraphics[width=10cm]{slike/difuzija-gompertzova-rast}
              \end{center}
              \caption{Vzeli smo $D=10$, $a=1$ in majno začetno porazdelitev, ki povzroči začetek rasti - slika a) \cite{kandler2010population}}
