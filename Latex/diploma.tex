...
Iz oblike histograma efektivnih polmerov (Slika \ref{fig:menisija-polmeri-hist}) se zdi smiselno, da ga prilegamo k standardni Maxwellovi porazdelitvi, čeprav za to nimamo globljih teoretičnih argumentov:
\begin{equation} 
...
\begin{equation}
  h(ri) = A \cdot e^{-\frac{(r-r_0)^2}{\sigma^2}} + C,
  \label{fit-vrtace}
\end{equation}
...
\begin{equation}
  w(L,t) \sim t^{1/2}.
\end{equation}
Zaradi korelacij med sosednjimi stolpci to obdbje ne traja dolgo, pač pa preide v obdobje rasti, kjer predpostavimo, da velja:
  \begin{equation}
...
\end{equation}
kjer je $\nu$ utež 'površinske napetosti', ki opisuje površinsko difuzijo. Ta člen očitno deluje "entropijsko" saj želi izravnavati stohastične variacije površine. Njegova vloga je podobna kot jo ima Fickov zakon pri difuziji delcev v raztopinah.
